...
const BlogDetailPage = ({ id }) => {
      ...
      const fetchBlog = async () => {
        try {
          ...
          const jsonLdmain = {
            ...
            '@type': 'Blog',
            name: data.title,
            image: `https://m3xtrader.com/coupon/uploads/${data.image}` ,
  ...
  if (loading) {
    return <div className="text-center h-screen"><Loader/></div>;
  }
  ...
  const breadcrumbs = [
    {name:"Home", url: '/'},
    {name: "blog",url:"/blog"},
    {name: `${blog.title?.substring(0,30)} ...`,url:`/blog/${blog.web_slug}`},
    ...
    <BreadCrumbs breadCrumbs={breadcrumbs}/>
      <div className="container bg-white h-full mx-auto py-4 px-4 sm:px-6 lg:px-8">
        <div className="lg:flex  lg:space-x-8">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-normal mb-4 lg:mb-5">
          {blog.title}
        </h1>
            <Image
                 width={1000}
              ...
              alt={blog.title}
              className="w-full h-[200px] sm:h-[300px] lg:h-[500px] object-cover mb-4"
            />

            {/* Display the sanitized blog description */}
            <div
              className="text-lg sm:text-base text-justify text-gray-700 mt-4"
