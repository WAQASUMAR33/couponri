...
const NewBlogCategorySlider = () => {
  ...
  useEffect(() => {
    fetch('/api/blogcategory') // Adjust the path as necessary
  ...
  useEffect(() => {
    if (firstCategoryRef.current) {
      // Update the reference width when it becomes available
      const updateWidth = () => {
        if (firstCategoryRef.current) {
          firstCategoryRef.current.style.width = `${firstCategoryRef.current.offsetWidth}px`;
        }
      };
      updateWidth();
      window.addEventListener('resize', updateWidth); // Update width on resize to accommodate responsive changes
      ...
      <div className="overflow-hidden" ref={sliderRef}>
        <div className="flex">
          {categories.map((category, index) => (
            ...
            >
               <a href={`/blogcategories/${category.web_slug}`}>
           ...
           src={`https://couponri.divenclave.com/uploads/${category.image}`} alt={category.title} className="h-32 w-32 rounded-full object-cover"/>
              </div>
