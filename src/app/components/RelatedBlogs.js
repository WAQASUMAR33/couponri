      ...
      {loadingStores ? (
        ...
        <ul>
          {similarStores.map((store) => {
            const sanitizedCompanyDescription = DOMPurify.sanitize(store.comp_description, {
              FORBID_TAGS: ['style'],
            });
            ...
            return (
              <li key={store.id} className="mb-4 flex items-start">
                ...
                />
                <div>
                  <Link href={`/store/${store.web_slug}`} className="block text-blue-500 hover:underline font-semibold">
                    {store.com_title}
                  </Link>
                  <div
                    className="text-gray-700 text-sm overflow-hidden text-ellipsis"
                    ...
                    dangerouslySetInnerHTML={{ __html: sanitizeDescription(store.comp_description) }}
                  ></div>
                </div>
              </li>
            );
          ...
          {similarOffers.map((offer) => (
            <li key={offer.id} className="mb-4 flex items-start">
              <Image
                ...
                src={`https://couponri.divenclave.com/uploads/${getCompanyLogo(offer.comp_id)}`}
                alt={offer.offer_title}
                className="w-16 h-16 object-cover rounded-lg mr-4"
              />
              <div>
                <Link href={`/store/${getCompanyslug(offer.comp_id)}`} className="block text-blue-500 hover:underline font-semibold">
                  {offer.offer_title}
                </Link>
