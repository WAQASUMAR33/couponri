import React, { useEffect, useState, useRef } from 'react';
import { useRouter } from 'next/navigation'; // This is the correct import for Next.js router
import { MdChevronLeft, MdChevronRight } from 'react-icons/md'; // Importing Material Design icons
import Loader from '../Loader';
import Image from "next/image";

const FeaturedStores = () => {
  const [stores, setStores] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const sliderRef = useRef(null);
  const router = useRouter();

  useEffect(() => {
    const fetchStores = async () => {
      try {
        const response = await fetch('/api/company');
        if (!response.ok) {
          throw new Error('Failed to fetch stores data');
        }

        const storesData = await response.json();
        const featuredStores = storesData.filter(store => store.comp_status === 'Featured');
        setStores(featuredStores);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching stores:', error);
        setError(error.message);
        setLoading(false);
      }
    };

    fetchStores();
  }, []);

  const scroll = (direction) => {
    if (sliderRef.current) {
      const { scrollLeft, clientWidth } = sliderRef.current;
      const scrollTo = direction === 'left' ? scrollLeft - clientWidth : scrollLeft + clientWidth;
      sliderRef.current.scrollTo({ left: scrollTo, behavior: 'smooth' });
    }
  };

  if(loading){
      return <div className='h-52'><Loader/></div>
    }

  if (error) {
    return <div>{`Error: ${error}`}</div>;
  }

  return (
    <div className="py-12 bg-gray-50">
      <h2 className="text-3xl font-bold text-center mb-8">Featured Stores</h2>
      <div className="relative">
        <button className="absolute left-0 z-10 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-90 transition-opacity duration-300 p-2 rounded-full" onClick={() => scroll('left')}>
          <MdChevronLeft size={30} />
        </button>
        <div className="overflow-hidden" ref={sliderRef} style={{ scrollBehavior: 'smooth' }}>
          <div className="flex">
            {stores.map((store) => (
              <div key={store.id} className="flex-shrink-0 p-4 flex justify-center items-center" style={{ width: '20%' }}>
                <a href={`/store/${store.web_slug}`}>
                  <Image
                 width={1000}
                  height={1000}
                  placeholder="data:image/jpeg;base64,"
          
                    src={`https://couponri.divenclave.com/uploads/${store.comp_logo}`}
                    alt={store.com_title}
                      className="w-24 h-24 sm:w-32 sm:h-32 object-contain bg-white shadow-md rounded-full hover:shadow-xl"
                  />
                </a>
              </div>
            ))}
          </div>
        </div>
        <button className="absolute right-0 z-10 top-1/2 -translate-y-1/2 bg-white bg-opacity-70 hover:bg-opacity-90 transition-opacity duration-300 p-2 rounded-full" onClick={() => scroll('right')}>
          <MdChevronRight size={30} />
        </button>
      </div>
      {stores.length > 5 && (
        <div className="flex justify-center mt-8">
          <button onClick={() => router.push('/store')} className="px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors duration-300">
            Show More
          </button>
        </div>
      )}
    </div>
  );
};

export default FeaturedStores;
